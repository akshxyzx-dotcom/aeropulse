...
function App() {
    ...
    async function loadDashboard() {
      try {
        ...
        const [
          ...
          fetch(`${API_BASE_URL}/api/summary?mode=real`),
          fetch(`${API_BASE_URL}/api/index-history?mode=real`),
          fetch(`${API_BASE_URL}/api/route-comparison?mode=real`),
          fetch(`${API_BASE_URL}/api/booking-horizon?mode=real`)
        ]);
        ...
        const horizonJson = await horizonResponse.json();

        setSummary(summaryJson);

        // Convert API history into chart format
        setIndexData(
        ...
        // Convert route API data into chart format
        setRouteData(
          routeJson.map((item) => ({
        ...
        // Convert horizon API data into chart format
        setHorizonData(
          horizonJson.map((item) => ({
    ...
    async function loadRouteAnalysis() {
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/route-analysis?origin=${from}&destination=${to}&mode=real`
        );
  ...
  React.useEffect(() => {
    async function loadPrediction() {
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/route-prediction?origin=${from}&destination=${to}&mode=real`
        );
        ...
        if (data.predicted_index > data.current_index) {
          trend = "UPWARD TREND";
        } else if (data.predicted_index < data.current_index) {
          trend = "DOWNWARD TREND";
        }

        const difference = Math.abs(
          data.predicted_index - data.current_index
        );
    ...
    return (
      <div className="dashboard">
        <div className="header">
          <h1>AEROPULSE</h1>
          <p>Airfare Price Intelligence</p>
        </div>
    ...
    <div className="dashboard">
      ...
      <section className="kpi-container">
        ...
        <div className="kpi-card">
          ...
          <strong>
            {summary.current_index.toFixed(2)}
          </strong>
        </div>
        ...
        <div className="kpi-card">
          ...
          <strong>
            ₹{summary.average_fare.toLocaleString()}
          </strong>
        </div>
      ...
      <section className="content">
        ...
        <div className="panel horizon-panel">
          ...
          <div className="panel-heading">
            <h2>Booking Horizon</h2>
            <p>Price Trend vs Average</p>
          </div>
